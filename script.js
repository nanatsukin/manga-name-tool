...
createApp({
    setup() {
        ...
        const exportData = async (format = 'png', optSettings = null) => {
            ...
            if (targetPageIndices.length === 0) return;

            isExporting.value = true;
            isProcessing.value = true;
            exportProgress.value = 0; // 進捗リセット
            ...
            for (let i = 0; i < targetPageIndices.length; i++) {
                ...
                exportStatusText.value = `Page ${pageIdx + 1} を処理中... (${i + 1}/${targetPageIndices.length})`;
                exportProgress.value = Math.round((i / targetPageIndices.length) * 100);

                // DOM要素の取得
                const el = document.getElementById(`render-page-${pageIdx}`);
                if (!el) continue;
                ...
                try {
                    ...
                    const dataUrl = await htmlToImage.toPng(el, {
                        ...
                        height: canvasH,
                        style: {
                            transform: `scale(${canvasW / el.clientWidth})`,
                    ...
                    if (format === 'pdf') {
                        if (!pdfDoc) {
                            pdfDoc = new jsPDF({ orientation: canvasW > canvasH ? 'l' : 'p', unit: 'px', format: [canvasW, canvasH] });
                        } else {
                    ...
                    // 【重要】メモリ解放：生成した DataURL を明示的に破棄
                    // 文字列長制限エラーを避けるため、使い終わったらすぐ null にする
                    URL.revokeObjectURL(dataUrl);

                    // ブラウザに休息を与えてガベージコレクションを促す
                    await new Promise(resolve => setTimeout(resolve, 100));

                } catch (e) {
                    console.error(`Error at page ${pageIdx + 1}:`, e);
                }
            }

            if (format === 'pdf' && pdfDoc) {
                exportStatusText.value = "PDFファイルを生成中...";
                pdfDoc.save("manga_project.pdf");
            }

            exportProgress.value = 100;
            exportStatusText.value = "完了しました";
            isExporting.value = false;
            isProcessing.value = false;

            // 最後に数秒待って表示を消す
            setTimeout(() => { exportProgress.value = 0; exportStatusText.value = ''; }, 3000);
        };
